...
final class LightingTest: XCTestCase {
    ...
    func testClosestObject1() {
        let objects = [
            (Sphere(center: Point3D(x: 0, y: 0, z: 10), radius: 5)),
            (Sphere(center: Point3D(x: 0, y: 0, z: 20), radius: 18))
        ]

        let lighting = Scene.getClosestIntersection(withObjects: objects, usingRay: Ray(
                startPoint: Point3D(x: 0, y: 0, z: 0),
    ...
    func testClosestObject2() {
        let objects: [Object3D] = [
            Sphere(center: Point3D(x: 0, y: 0, z: 10), radius: 5),
            Sphere(center: Point3D(x: 0, y: 0, z: 20), radius: 18),
            Plane(point: Point3D(x: 0, y: 0, z: 1), normal: Vector3D(x: 0, y: 0, z: 1))
        ]
